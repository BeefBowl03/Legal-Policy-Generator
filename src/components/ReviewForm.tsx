    ...
    <div className="space-y-8">
        ...
        {Object.entries(groupedQuestions).map(([groupName, groupQuestions]) => (
          <div key={groupName} className="premium-card">
            <div className="px-6 py-4 border-b border-border-subtle">
              <h3 className="text-lg font-semibold text-text-primary">{groupName}</h3>
            </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="mt-2 sm:mt-0 sm:ml-4">
                      {question.required && !formData[question.field] && (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-error-light/20 text-error-dark border border-error-dark/30">
                          Required
                        </span>
                      )}
                      {formData[question.field] && (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-success-light/20 text-success-dark border border-success-dark/30">
                          ✓ Answered
                        </span>
                      )}
                    </div>
      ...
      <div className="flex items-center justify-between pt-6">
        ...
        <button
          type="button"
          onClick={onComplete}
          className="px-8 py-3 text-base font-medium text-black bg-gradient-to-r from-primary-500 to-primary-400 border border-transparent rounded-lg hover:from-primary-600 hover:to-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:ring-offset-background-primary shadow-elevated hover:shadow-premium transition-all duration-200 transform hover:scale-105"
        >
